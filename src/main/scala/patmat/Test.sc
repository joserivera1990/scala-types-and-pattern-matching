...
object Test {
  ...
  def times1(chars: List[Char]): List[(Char, Int)] = {
  ...
  val t1 = times1("abcddd" toList)

  def times2(chars: List[Char]): List[(Char, Int)] = {
    chars.groupBy(identity).mapValues(_.size).toList
  }
  val chars11 = "abcddd"
  val test = chars11.groupBy(identity).mapValues(_.size).toList
  val t2 = times1("abcddd" toList)
  ...
  def makeOrderedLeafList1(freqs: List[(Char, Int)]): List[Leaf] = {
  ...
  val p = makeOrderedLeafList(times2("abbcdddaaa" toList))
  val p1 = makeOrderedLeafList1(times2("abbcdddaaa" toList))
